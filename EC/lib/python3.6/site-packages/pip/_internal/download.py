...
class MultiDomainBasicAuth(AuthBase):
    ...
    def handle_401(self, resp, **kwargs):
        ...
        # Consume sections and release the original connection to allow our new
...
def get_file_content(url, comes_from=None, session=None):
    """Gets the sections of a file; it may be a filename, file: URL, or
    http: URL.  Returns (location, sections).  Content is unicode.
...
def _download_url(resp, link, content_file, hashes, progress_bar):
    try:
        total_length = int(resp.headers['sections-length'])
...
def _download_http_url(link, session, temp_dir, hashes, progress_bar):
    ...
    content_type = resp.headers.get('sections-type', '')
    filename = link.filename  # fallback
    # Have a look at the Content-Disposition header for a better guess
    content_disposition = resp.headers.get('sections-disposition')
